...
from wenet.utils.executor_ts import TeacherStudentExecutor
...
def get_args():
    ...
    parser.add_argument('--teacher_config', required=True, help='teacher model config file')
    parser.add_argument('--teacher_checkpoint', required=True, help='teacher model final config file')
...
def main():
    ...
    executor = TeacherStudentExecutor()
    # If specify checkpoint, load some info from checkpoint
    if args.checkpoint is not None:
        infos = load_checkpoint(model, args.checkpoint, args.restore_encoder_only, force_cpu=True)
    else:
        infos = {}
    ...
    teacher_infos = load_checkpoint(teacher_model, args.teacher_checkpoint, False, force_cpu=True)

    join_model = init_ts_asr_model(teacher_model, model, configs)
    model = join_model
    ...
    if configs['optim'] == "novograd":
        ...
        optimizer = optim_ex.NovoGrad(model.module.student.parameters(), lr=configs['optim_conf']['lr'], betas=(0.8, 0.25))
        scheduler = WarmupLR(optimizer, **configs['scheduler_conf'])
    elif configs['optim'] == "radam":
        print("preparing radam optimizer")
        optimizer = optim.RAdam(model.module.student.parameters(), **configs['optim_conf'])
        scheduler = WarmupLR(optimizer, **configs['scheduler_conf'])
        ...
        if distributed:
            optimizer = optim.Adam(model.module.student.parameters(), **configs['optim_conf'])
        else:
            optimizer = optim.Adam(model.student.parameters(), **configs['optim_conf'])
    ...
    if start_epoch == 0 and args.rank == 0:
        ...
        if distributed:
            save_checkpoint(model.module.student, save_model_path, optimizer=optimizer)
        else:
            save_checkpoint(model.student, save_model_path, optimizer=optimizer)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if args.world_size > 1:
            ...
            cv_num_seen_utts = torch.Tensor([cv_num_seen_utts]).to(device)
            ...
            att_acc = total_att_acc[0] / cv_num_seen_utts[0]
    ...
    if final_epoch is not None and args.rank == 0:
        ...
        if os.path.exists(final_model_path):
            os.move(final_model_path, final_model_path + '.bak')
