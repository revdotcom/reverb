...
class PenaltyBuilder(object):
    ...
    def __init__(self, cov_pen: Optional[str], length_pen: Optional[str]):
        self.has_cov_pen = not self._pen_is_none(cov_pen)
        self.coverage_penalty = cov_pen #self._coverage_penalty(cov_pen)
        self.has_len_pen = not self._pen_is_none(length_pen)
        self.length_penalty = length_pen #self._length_penalty(length_pen)

    #@staticmethod
    def _pen_is_none(self, pen: Optional[str]):
        return pen is None or pen == "none"

    """
    def _coverage_penalty(self, cov_pen: Optional[str]):
        if self._pen_is_none(cov_pen):
    ...
    def _length_penalty(self, length_pen: Optional[str]):
        if self._pen_is_none(length_pen):
    ...
    def coverage_wu(self, cov, beta: float=0.0):
    ...
    def coverage_summary(self, cov, beta:float=0.0):
        """Our summary penalty."""
        penalty = torch.max(cov, cov.clone().fill_(1.0)).sum(-1)
        penalty -= cov.size(-1)
        return beta * penalty

    def coverage_none(self, cov, beta:float=0.0):
        """Returns zero as penalty"""
        none = torch.zeros((1,), device=cov.device, dtype=torch.float)
        if cov.dim() == 3:
            none = torch.zeros((cov.shape[0], 1,), device=cov.device, dtype=torch.float)
        return none

    def length_wu(self, cur_len: int, alpha:float=0.0):
    ...
    def length_average(self, cur_len: int, alpha:float=1.0):
        """Returns the current sequence length."""
        return cur_len**alpha

    def length_none(self, cur_len: int, alpha:float=0.0):
