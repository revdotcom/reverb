...
def joint_decoding(
        ...
        try:
            decoder = model.decoder.left_decoder
        except:
        ...
        for b in range(batch_size):
            ...
            ctc_prob = ctc_probs[b, :encoder_lens[b], :]
            ...
            n_best_hyps, n_best_scores, start_times, end_times, n_best_confs = beam_search(x = encoder_out,
                                                                                           cat_embs = cat_embs)
            results.append(
                DecodeResult([h.item() for h in n_best_hyps[0][1:]],
                             n_best_scores[0].item(),
                             times=[t.item() for t in start_times[0][0,1:]],
                             tokens_confidence=[math.exp(c.item()) for c in n_best_confs[0][1:]]))
